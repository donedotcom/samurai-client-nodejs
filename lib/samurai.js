...
var samurai-client-nodejsError = require('./error');
  ...
  if (opts) {
    if (typeof opts.token !== 'undefined') {
      if (!tokenRe.test(opts.token)) {
        throw new samurai-client-nodejsError('system', 'Invalid token', null);
      } 
      ...
      if (!self.number) {
        throw new samurai-client-nodejsError('system', 'Card number is required', null);
      }
      if (!self.csc) {
        throw new samurai-client-nodejsError('system', 'CSC is required', null);
      }
...
Card.prototype.create = function(callback) {
  ...
  authpost.makeRequest({
    ...
    if (err && err instanceof samurai-client-nodejsError) {
      callback(err);
    }
    if (err) {
      callback(new samurai-client-nodejsError('system', 'Error making create payment method request', err));
      return;
    }
    if (res.status !== 302) {
      // Parse the body to find the error
      callback(new samurai-client-nodejsError(
        'system', 
    ...
    if (!res.headers.location) {
      callback(new samurai-client-nodejsError('system', 'Gateway failed to send the location header', res.headers));
      return;
    }
...
Card.prototype._rejectNoToken = function(callback) {
  var error = new samurai-client-nodejsError('system', 'Cannot update payment method without token', null);
